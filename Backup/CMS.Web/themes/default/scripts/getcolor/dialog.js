...
function relativePath2rootPath(url){
	...
	var sEditorPath = getEditorRootPath();
	while(url.substr(0,3)=="../"){
		url = url.substr(3);
		sEditorPath = sEditorPath.substring(0,sEditorPath.lastIndexOf("/"));
	}
	return sEditorPath + "/" + url;
}
...
function baseHref2editorPath(url){
	var editorPath = getEditorRootPath() + "/";
...
function SelectFiles(objname,scid,stype){
  var arr=showModalDialog('../SelFiles.asp?CID=' + scid + '&Type=' + stype + '', '', 'dialogWidth:700px; dialogHeight:500px; help: no; scroll: yes; status: yes');
	if(arr!=null){
		var obj = document.getElementById(objname);
		var strT=arr.split('|');
		obj.value=strT[0];
		if ((IsExt(strT[0],'gif|bmp|jpg|png')) && (stype=="0")){
			doPreview();
		}
	}
}