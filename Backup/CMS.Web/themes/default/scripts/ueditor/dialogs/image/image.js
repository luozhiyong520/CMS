...
{
     // window.open ('/media/imagelist.aspx','newwindow','height=600,width=800,top=100,left=260,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no') 
    // dialog.close();
     var returnValue = window.showModalDialog("/media/imagelist.aspx", 'newwindow', "dialogHeight=620px;dialogWidth=920px;dialogLeft=260px;dialogTop=100px");

     if(returnValue != null)
     {
    // editor.execCommand("insertHtml",returnValue)
    var strs = new Array(); //定义一数组
    strs = returnValue.split("&"); //字符分割 
    for (i = 0; i < strs.length; i++) {
         editor.execCommand("insertImage",{
            src: "http://img.upchina.com"+strs[i],          //图片链接地址,必选
            data_ue_src:strs[i],  //UE内部使用参数，与src保持一致即可，可选
           // width: 300,                                        //图片显示宽度，可选
