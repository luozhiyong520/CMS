             ...
             <TR>
              <TD>
                ...
                <select id="LiveType" name="LiveType">
                 <%if (Model.AnalystLive.LiveType == 1)
                   { %>
                    <option value="1" selected="selected" >开仓</option>
                 <%}
                   else if (Model.AnalystLive.LiveType == 2)
                   { %>
                  <option value="2" selected="selected" >平仓</option>
                  <%}
                   else if (Model.AnalystLive.LiveType == 3)
                   {%>
                    <option value="3" selected="selected" >观点</option>
                    <%} %>
                </select> <font color="red"> *必选</font>
               &nbsp;&nbsp;商品名称:<INPUT name="ActualName" id="ActualName" size="30" maxlength="50" class= "inTextbox" disabled=disabled  value="<%=Model.AnalystLive.ActualName%>" <%if (Model.AnalystLive.LiveType==2){%>disabled="disabled"<%} %>  /><font color="red" class="bx"> *必选</font>
                   <ul style="display:none; position:absolute; left:190px" id="showpclist"  class="showpclist">
                   </ul>
                   
                <INPUT type="hidden" name="ActualCode" id="ActualCode" size="12" maxlength="50" class= "inTextbox"  value="<%=Model.AnalystLive.ActualCode%>"/>
                &nbsp;&nbsp;价格:<INPUT name="TransPrice" id="TransPrice" size="10" maxlength="10" class= "inTextbox"  value="<%=Model.AnalystLive.TransPrice%>"/><font color="red"  class="bx"> *必填</font>
                &nbsp;&nbsp;类型:
                 <select id="TransType" name="TransType">
                    <option value="0"  <%if (Model.AnalystLive.LiveType == 3){ %>disabled=disabled<%} %>>选择类型</option>
                   <%if (Model.AnalystLive.LiveType == 1 && Model.AnalystType==1){ %>
                      <option value="买入订立" id="mrdl" <%if(Model.AnalystLive.TransType=="买入订立"){ %>selected<% }%>>买入订立</option>
                      <option value="卖出订立" id="mcdl" <%if(Model.AnalystLive.TransType=="卖出订立"){ %>selected<% }%>>卖出订立</option>
                    <%}else if(Model.AnalystLive.LiveType == 1 && Model.AnalystType==2){ %>
                    <option value="买入开仓" id="mrkc" <%if(Model.AnalystLive.TransType=="买入开仓"){ %>selected<% }%>>买入开仓</option>
                    <option value="卖出开仓" id="mckc" <%if(Model.AnalystLive.TransType=="卖出开仓"){ %>selected<% }%>>卖出开仓</option>
                  <%}else if (Model.AnalystLive.LiveType == 2){ %>
                      <option value="<%=Model.AnalystLive.TransType%>"  selected><%=Model.AnalystLive.TransType%></option>
                    <%} %>
               ...
               &nbsp;&nbsp;发布时间:<INPUT name="CreateTime"  id="CreateTime" size="30" maxlength="50" class= "inTextbox"  value="<%=Model.AnalystLive.CreateTime%>"/>
               </TD>
            </TR>
              ...
              <tr>
               ...
               <tr>
               <TD style="padding:6px 10px;">
                 &nbsp;&nbsp;超链接:<INPUT name="LinkUrl" id="LinkUrl" size="100" maxlength="100" class= "inTextbox"  value="<%=Model.AnalystLive.LinkUrl%>"/> <font color=red>请以http://开头</font>
               </TD>
               ...
               <TD style="padding:6px 10px;" align="center" >
                    <input type="hidden" name="LiveId" id="LiveId" value="<%=Model.AnalystLive.LiveId%>" />
                    ...
                    <input type="hidden" name="pc_LiveId" id="pc_LiveId" value="<%=Model.AnalystLive.BuyLiveId%>"/>
                    <input type="hidden" name="IsSell" id="IsSell" value="<%=Model.AnalystLive.IsSell%>"/>
                    <input type="hidden" name="operate" value="edit" id="operate" />
                  ...
                  <input type="button" value="发布" class="inButton" id="Editzbinfo" />
               </TD>
            </tr>
            </TABLE>
        </div>
    ...
    <script language="javascript" type="text/javascript">
        ...
        $(function () {
         self.parent.document.body.style.overflow = 'hidden';
           ...
           if(<%=Model.AnalystLive.LiveType%>==3)
           {
            ...
            $("#Editzbinfo").click(function () {
              ...
              var info = encodeURIComponent(editor.getContent().replace(new RegExp("&#39;", 'gm'), ""));
                ...
                $("#Editzbinfo").val("正在提交中...");
                $("#Editzbinfo").attr("disabled", true);
