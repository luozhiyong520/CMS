...
function Loaddrop() {
    $.ajax({
        url: '/AjaxAuthorityDot/GetAuthoritydot.cspx?productId=1',
        success: function (shtml) {
            $("#slctPushVersion").html('');
            $("#slctPushVersion").append("<option value=\"请选择\">请选择</option>" + shtml);
        }
    });
}
function LoaddropLook() {
    $.ajax({
        url: '/AjaxAuthorityDot/GetAuthoritydotLook.cspx',
        success: function (shtml) {
            $("#slctPushVersionLook").html('');
            $("#slctPushVersionLook").append("<option value=\"请选择\">请选择</option>" + shtml);
        }
    });
}
$(function () {
    Loaddrop();
    LoaddropLook();
    if (getQueryString("PlanId") == "" || getQueryString("PlanId") == undefined) {
    } else {
    ...
    $("#submit").click(function () {
        ...
        var PushVersion = $("#slctPushVersion").val();
        var PushColumn = $("#slctPushColumn").val();
        var PushPlatform = "";
        $("#PushPlatform input:checked").each(function () {
            if (PushPlatform == "") {
                PushPlatform = $(this).val();
            } else {
                PushPlatform += "," + $(this).val();
            }
        });
        ...
        //alert(BeginTime + "=" + nowTime);
        if (BeginTime != "") {
            if (comptime(nowTime, BeginTime)) {
                alert("开始时间必需大于当前时间!");
        ...
        if ($("#PushPlatform input:checked").length < 1) {
            alert("请选择平台!");
            $("#submit").attr("disabled", false);
            return false;
        }
...
//            person.html = "<span class=\"l-btn-left\"><span class=\"l-btn-text l-btn-focus\">已推送</span></span>";       
        }
    });
}
...
function lookpopup(PlanId) {
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            $("#slctPushVersionLook option[value='" + json.PushVersion + "']").attr("selected", "selected");
            $("#slctPushColumn").val(json.PushColumn);
            if (datatype == "look") {
                if (json.ImgUrl ==null) {
                } else {
                    $("#imgurlId").html("");
                    var shtml = "<td align=\"right\">图片路径:</td><td><input disabled=\"disabled\" type=\"text\" value=\"" + json.ImgUrl + "\" name=\"textfield\" id=\"ImgUrl\" style=\" width:350px;\" class=\"w300 inTextbox\" />";
                    $("#imgurlId").append(shtml);
                }
            }
            if (json.PushPlatform == "PC") {
                $("#chboxPushPlatformPC").attr("checked", "checked");
            } else if (json.PushPlatform == "Move") {
                $("#chboxPushPlatformMove").attr("checked", "checked");
            }
            else {
                $("#chboxPushPlatformPC").attr("checked", "checked");
                $("#chboxPushPlatformMove").attr("checked", "checked");
                    ...
                    success: function (res) {
                        ...
                        for (var i = 0; i < groups.length; i++) {
                            ...
                            if (datatype == "add") {
                                ...
                                groupHtml += "<li><label><input disabled=\"disabled\" type=\"checkbox\" name=\"ReceiverGroup\" value=\"" + groups[i] + "\" " + checked + "/> " + groups[i] + "</label></li>";
                            }
