...
function insertImage() {
    ...
    if (window.ActiveXObject) { //IE
        ...
        var returnValue = window.showModalDialog("/media/imagelist.aspx", window, "dialogWidth:850px;status:no;dialogHeight:600px");
        if (returnValue != null) {
            setValue(returnValue.name);
        }

    } else {  //非IE
        window.open("/media/imagelist.aspx", 'newwindow', 'height=600,width=850,top=150,left=300,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
    ...
    if (returnValue != null) {
        ...
        for (i = 0; i < strs.length; i++) {
            ...
            editor.execCommand("insertImage", {
                src: "http://img.pinyifu.com" + strs[i],          //图片链接地址,必选
                data_ue_src: "http://img.pinyifu.com" + strs[i],  //UE内部使用参数，与src保持一致即可，可选
                // width: 300,                                        //图片显示宽度，可选
