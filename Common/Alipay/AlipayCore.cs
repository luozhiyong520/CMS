...
namespace Common
{
    ...
    public class Core
    {
        ...
        public static string CreateLinkString(Dictionary<string, string> dicArray)
        {
            ...
            {
                prestr.AppendFormat("{0}={1}&", item.Key, item.Value);
            }
        ...
        public static string CreateLinkStringUrlencode(Dictionary<string, string> dicArray, Encoding code)
        {
            ...
            foreach (KeyValuePair<string, string> item in dicArray)
            {
                prestr.AppendFormat("{0}={1}&", item.Key, HttpUtility.UrlEncode(item.Value, code));
            }
        ...
        {
            string strPath = HttpContext.Current.Server.MapPath("/Alipay_log");
            strPath = strPath + "\\" + DateTime.Now.ToShortDateString().Replace("-", "").Replace("/","") +".log"; // '/'有的服务器时间可能设置成这种格式
